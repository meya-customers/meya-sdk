...
@dataclass
class ShopifyCustomerSearchComponent(BaseApiComponent):
    """
    ...
    class Response(ApiComponentResponse):
        result: List[Customer] = response_field(sensitive=True)

    email: str = element_field(help="Customer email")
    ...
    async def start(self) -> List[Entry]:
        ...
        with ShopifyIntegration.current.set(integration):
            response = await integration.customer_search(self.email)
