...
@dataclass
class ShopifyProductGetComponent(BaseApiComponent):
    """
    Get product
    """
    ...
    class Response(ApiComponentResponse):
        result: Product = response_field(sensitive=True)

    product_id: int = element_field(help="Product ID")
    ...
    async def start(self) -> List[Entry]:
        ...
        with ShopifyIntegration.current.set(integration):
            response = await integration.product_get(
